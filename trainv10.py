...
def train_fn(device = "cuda:0", load_state = False, state_path = './'):
    ...
    for epoch in range(0, num_epochs):
        ...
        for i , (input,target) in enumerate(tqdm(train_loader, total=len(train_loader))):
            ...
            optim.zero_grad()
            output, output_sup1, output_sup2, output_sup3 = model(img)
            ...
            loss1 = criterion(output, depth, mask)
            loss2 = criterion(output_sup1, depth, mask)
            loss3 = criterion(output_sup2, depth, mask)
            loss4 = criterion(output_sup3, depth, mask)
...
if __name__ == "__main__":
    train_fn(device='cuda:0', load_state=False, state_path="/home/gremsy_guest/hyp_workspace/depth_v2/ours_checkpoints/12")