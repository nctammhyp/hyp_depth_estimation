...
def compute_depth_loss(y_pred, y_true, device="cuda", alpha=5.0, beta=1.0, theta=2.0):
    """
    Compute total depth estimation loss:
      total_loss = (alpha * L1 + beta * SSIM + theta * Gradient) / (alpha + beta + theta)

    Args:
        y_pred: predicted depth [B,1,H,W]
        y_true: ground truth depth [B,1,H,W]
        device: 'cuda' or 'cpu'
        alpha, beta, theta: loss weights

    Returns:
        total_loss: weighted loss value
        loss_dict: dictionary of individual components
    """
    # Ensure same dtype/device
    y_pred = y_pred.to(device, dtype=torch.float32)
    ...
    # --- SSIM loss ---
    ssim_val = ssim(y_pred, y_true)
    # Convert similarity to loss
    ssim_loss = torch.clamp((1 - ssim_val) * 0.5, 0, 1)

    # --- Gradient loss ---
    grad_loss = depth_loss(y_true, y_pred, device)

    # --- Weighted combination ---
    total_loss = alpha * l1_loss + beta * ssim_loss + theta * grad_loss
