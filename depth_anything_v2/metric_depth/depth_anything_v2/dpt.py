...
class DPTHead(nn.Module):
    ...
    def forward(self, out_features, patch_h, patch_w):
        ...
        out = self.activate_last(out)
        # for i, layer in enumerate(self.scratch.output_conv2):
        #     out = layer(out)
        #     print(f"[output_conv2.{i} -> {layer.__class__.__name__}] {tuple(out.shape)}")

        
        return out, teacher_feat_mapped
...
class DepthAnythingV2(nn.Module):
    ...
    def forward(self, x):
        ...
        depth, fea_distill = self.depth_head(features, patch_h, patch_w) 
        # depth = depth * self.max_depth
        
        return depth.squeeze(1), fea_distill
